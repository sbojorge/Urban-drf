...
class CommentListCreateViewTests(APITestCase):
    ...
    def test_authenticated_user_can_create_comment(self):
        self.client.login(username='ella', password='rabbit')
        post = Post.objects.get(id=1)
        response = self.client.post('/comments/', {'content': 'great post!', 'post': 1})
...
class CommentDetailViewTests(APITestCase):
    """
    Test retrieving a comment by id and updating/deleting the comment if the user owns it
    """
    def setUp(self):
        ella = User.objects.create_user(username='ella', password='rabbit')
        ...
        Comment.objects.create(owner=damien, post= post_d, content='cool post!')
    ...
    def test_authenticated_user_can_update_owned_comment(self):
        self.client.login(username='ella', password='rabbit')
        response = self.client.put('/comments/1/', {'content': 'super great post!'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    
    def test_user_cant_update_someoneelses_comment(self):
        self.client.login(username='ella', password='rabbit')
        response = self.client.put('/comments/2/', {'content': 'not a cool post'})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
